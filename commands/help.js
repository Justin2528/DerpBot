...
if(args[0] == "cool") return message.channel.send("No such command call `cool`");
  ...
  if(!args[0]){
    ...
    .addField(`Commands:`, "``serverinfo`` `help` `test2` `rps` `mute` `ban` `softban` `unban` `kick` `ping` `uptime` `???` `yesorno` `meme` `purge` `eval` `reload` `cat` `dog` `pitstats` `minesweeper` `invite` `npm` `1vs1` `vote`")
